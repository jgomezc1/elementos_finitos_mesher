...
def _cells_to_dict(cells):
    """Convert meshio cells list to dictionary format (for meshio v4.0+ compatibility)

    Parameters
    ----------
    cells : list
        List of CellBlock objects from meshio v4.0+

    Returns
    -------
    cells_dict : dict
        Dictionary with cell type as key and connectivity array as value
    """
    if isinstance(cells, dict):
        return cells  # Already in dict format (old meshio)
...
def _cell_data_to_dict(cell_data):
    """Convert meshio cell_data to dictionary format (for meshio v4.0+ compatibility)

    Parameters
    ----------
    cell_data : dict
        Cell data from meshio (format varies by version)

    Returns
    -------
    cell_data_dict : dict
        Dictionary with cell type as key and data dict as value
    """
    if not cell_data:
        return {}

    # Check if already in old format (nested dict with cell types as keys)
    first_key = list(cell_data.keys())[0]
    first_value = cell_data[first_key]

    if isinstance(first_value, dict):
        return cell_data  # Already in old format

    # New format: cell_data is {data_name: [array_per_cell_block]}
    # We need to convert to {cell_type: {data_name: array}}
    cell_data_dict = {}

    # We need access to cells to get the types - this will be handled in calling function
    return cell_data
